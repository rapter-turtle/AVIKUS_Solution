function [sys] = usv_dynamics_mpc_cmd(x,uu)
    P = load_parm();
    ...
    s = 25;
    k = 8;
    a1 = P.KPrvs * P.dead_rps;
    a2 = P.KPfwd * P.dead_rps;
    b1 = P.KPrvs;
    b2 = P.KPfwd;
   
    TP_state = (1/(1+exp(s*TP_cmd)))*(b1*TP_cmd + tanh(k*TP_cmd)*a1) + ...
        (1/(1+exp(-s*TP_cmd)))*(b2*TP_cmd + tanh(k*TP_cmd)*a2);
    TS_state = (1/(1+exp(s*TS_cmd)))*(b1*TS_cmd + tanh(k*TS_cmd)*a1) + ...
        (1/(1+exp(-s*TS_cmd)))*(b2*TS_cmd + tanh(k*TS_cmd)*a2);
