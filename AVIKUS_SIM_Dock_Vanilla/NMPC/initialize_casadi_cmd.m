function obj = initialize_casadi_cmd(Num,dt,Q,R,QN)
import casadi.*
USV_P = load_parm();
...
for k = 1:Num
    ...
    xdot = usv_dynamics_mpc_cmd(st, con);
    ...
    TP_prev = (1/(1+exp(s*X(7,k))))*(b1*X(7,k) + tanh(kk*X(7,k))*a1) + ...
        (1/(1+exp(-s*X(7,k))))*(b2*X(7,k) + tanh(kk*X(7,k))*a2);
    TS_prev = (1/(1+exp(s*X(8,k))))*(b1*X(8,k) + tanh(kk*X(8,k))*a1) + ...
        (1/(1+exp(-s*X(8,k))))*(b2*X(8,k) + tanh(kk*X(8,k))*a2);
    
    TP_cur = (1/(1+exp(s*X(7,k+1))))*(b1*X(7,k+1) + tanh(kk*X(7,k+1))*a1) + ...
        (1/(1+exp(-s*X(7,k+1))))*(b2*X(7,k+1) + tanh(kk*X(7,k+1))*a2);
    TS_cur = (1/(1+exp(s*X(8,k+1))))*(b1*X(8,k+1) + tanh(kk*X(8,k+1))*a1) + ...
        (1/(1+exp(-s*X(8,k+1))))*(b2*X(8,k+1) + tanh(kk*X(8,k+1))*a2);
    
    Q_d = 0.01;
    objective = objective + (TP_cur-TP_prev)'*Q_d*(TP_cur-TP_prev);
    objective = objective + (TS_cur-TS_prev)'*Q_d*(TS_cur-TS_prev);
