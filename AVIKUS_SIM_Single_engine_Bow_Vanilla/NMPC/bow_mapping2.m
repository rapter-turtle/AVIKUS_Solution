function new_bow_array = bow_mapping(bow_array,dt,Num, dwell_time)
    ...
    for i = 1:Num - 3
        % Left
        Aineq = [Aineq; [zeros(1, 2*i), -1, 0, 1, 0, -1, 0, zeros(1, total_variables - 2*i - 6)]];
        bineq = [bineq; eps];

        % Right
        Aineq = [Aineq; [zeros(1, 2*i), 0, -1, 0, 1, 0, -1, zeros(1, total_variables - 2*i - 6)]];
        bineq = [bineq; eps];

        for j = 2:dwell_count
            if i+j <= Num-2
                % Left
                Aineq = [Aineq; [zeros(1, 2*i), -1, 0, 1, 0, zeros(1, 2*(j-1)), -1, 0, zeros(1, total_variables - 2*i - 6 - 2*(j-1))]];
                bineq = [bineq; eps];

                % Right
                Aineq = [Aineq; [zeros(1, 2*i), 0, -1, 0, 1, zeros(1, 2*(j-1)), 0, -1, zeros(1, total_variables - 2*i - 6 - 2*(j-1))]];
                bineq = [bineq; eps];
            end
        end
    end
