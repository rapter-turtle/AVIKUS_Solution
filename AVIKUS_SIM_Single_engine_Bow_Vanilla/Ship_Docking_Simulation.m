...
Q = diag([1 1 1e-1 10 10 1000 1e-5 1]);
R = diag([1e-1 1 1e-3]);
R2 = diag([1e-3 1]);
QN = Num*diag([1e3 1e3 1e4 100 100 1000 1 1]);
...
for i = 2:N
    ...
    %% Dynamic updatae
    [u_state(i), v_state(i), r_state(i), x_state(i), y_state(i), psi_state(i), thr, del, rps] = update_ship_dynamics(u_state(i-1), v_state(i-1), r_state(i-1), x_state(i-1), y_state(i-1), psi_state(i-1), thr, del, T_cmd, del_cmd, T_bow_cmd, dt, WD, WS);
