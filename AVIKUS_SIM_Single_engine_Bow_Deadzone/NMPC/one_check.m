function new_bow_array = one_check(bow_array,dt,Num, dwell_time)
    ...
    % Turn on
    for i = 1:Num-2
        Aineq = [Aineq; [zeros(1, i),-1, 1, -1, zeros(1, Num + 1 - i - 3)]];
        bineq = [bineq; 0];

        for j = 1:dwell_count-1
            if (i + j + 3) < total_variables
                Aineq = [Aineq; [zeros(1, i), -1, 1, zeros(1, j), -1, zeros(1,total_variables - i - j - 3)]];
                bineq = [bineq; 0];
            elseif (i + j + 3) == total_variables
                Aineq = [Aineq; [zeros(1, i), -1, 1, zeros(1, j), -1]];
                bineq = [bineq; 0];            
            end
        end
    end
