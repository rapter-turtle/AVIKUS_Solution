...
Q = diag([1e1 1e1 1e3 1e3 1e3 1000 1e1 1]);
R = diag([1e1 1e2 1e-1]);
R2 = diag([1e1 1e2]);
QN = Num*diag([1e1 1e1 1e2 1000 1000 1000 1 1]);
...
v.FrameRate = 5; % 초당 프레임 수, 적절히 조정 가능
open(v);
end
...
for i = 2:N
    ...
    % if mod(i, control_update_steps) == 0
        ...
        % 
        % % Get control input
        % MPC_input = [usol(1,1), usol(1,2), T_bow_cmd]';
        ...
        nlp_bow_array = usol(:,3)/USV_P.bow_thrust;
        if (count_on_left <= dwell_time/dt -  1) && (T_bow_cmd > 0.1)
            count_on_left = count_on_left + 1;
            input_bow_array = bow_mapping(nlp_bow_array((dwell_time/dt) - count_on_left + 1:end),control_update_dt, length(nlp_bow_array((dwell_time/dt) - count_on_left + 1:end)), dwell_time, stop_dwell_time);
            bow_array = [ones((dwell_time/dt) - count_on_left, 1); input_bow_array];
            
            count_on_right = 0;
            count_on_zero = 0;
        
        elseif (count_on_right <= dwell_time/dt - 1) && (T_bow_cmd < -0.1)
            count_on_right = count_on_right + 1;
            input_bow_array = bow_mapping(nlp_bow_array((dwell_time/dt) - count_on_right + 1:end),control_update_dt, length(nlp_bow_array((dwell_time/dt) - count_on_right + 1:end)), dwell_time, stop_dwell_time);
            bow_array = [-ones((dwell_time/dt) - count_on_right, 1); input_bow_array];
            
            count_on_left = 0;
            count_on_zero = 0;

        elseif (count_on_zero <= stop_dwell_time/dt - 1) && (T_bow_cmd == 0.0)
            count_on_zero = count_on_zero + 1;
            input_bow_array = bow_mapping(nlp_bow_array((stop_dwell_time/dt) - count_on_zero + 1:end),control_update_dt, length(nlp_bow_array((stop_dwell_time/dt) - count_on_zero + 1:end)), dwell_time, stop_dwell_time);
            bow_array = [zeros((stop_dwell_time/dt) - count_on_zero, 1); input_bow_array];
            ...
            count_on_right = 0;
            count_on_zero = 0;
            input_bow_array = nlp_bow_array;
            bow_array = bow_mapping(input_bow_array,control_update_dt, Num, dwell_time, stop_dwell_time);
        end
        ...
        T_cmd = xsol2(1,7) + d_T_cmd*dt;
        del_cmd = xsol2(2,8) + d_del_cmd*dt;
        T_bow_cmd = bow_array(1)*USV_P.bow_thrust;
        ...
        % Get control input
        MPC_input = [usol(1,1), usol(1,2), T_bow_cmd]';
        MPC_input2 = [usol2(1,1), usol2(1,2)]';
